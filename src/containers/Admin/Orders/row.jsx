  ...
  return (
    <React.Fragment>
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        <TableCell>
        <Select options={status.filter(status=>status.value !=='Todos')} menuPortalTarget={document.body} placeholder='Status' defaultValue={status.find(option=>option.value === row.status) ||null} onChange={valueStatus=>setNewStatus(row.id,valueStatus.label)} isLoading={load}/> 
        </TableCell>
      </TableRow>
      <TableRow>
